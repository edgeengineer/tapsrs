...
fn test_message_lifetime() {
    let msg = Message::from_string("Test")
...
fn test_message_priority() {
    ...
    let low_priority = Message::from_string("Low priority")
        .with_priority(-50);
...
fn test_message_ordered() {
    let ordered_msg = Message::from_string("Ordered")
        .with_ordered(true);
    assert_eq!(ordered_msg.properties().ordered, Some(true));
    
    let unordered_msg = Message::from_string("Unordered")
        .with_ordered(false);
...
fn test_message_safely_replayable() {
    let replayable = Message::from_string("Replayable")
        .safely_replayable();
...
fn test_message_final() {
    let final_msg = Message::from_string("Final")
        .final_message();
...
fn test_message_checksum_length() {
    let msg = Message::from_string("Checksummed")
        .with_checksum_length(16); // 16 bytes = 128 bits
...
fn test_message_reliable() {
    let reliable = Message::from_string("Reliable")
        .with_reliable(true);
    assert_eq!(reliable.properties().reliable, Some(true));
    
    let unreliable = Message::from_string("Unreliable")
        .with_reliable(false);
...
fn test_message_capacity_profile() {
    ...
    let bulk = Message::from_string("Bulk")
        .with_capacity_profile(MessageCapacityProfile::Scavenger);
    assert_eq!(
...
fn test_message_fragmentation_control() {
    let no_frag = Message::from_string("No fragmentation")
        .no_fragmentation();
    assert!(no_frag.properties().no_fragmentation);
    
    let no_seg = Message::from_string("No segmentation")
        .no_segmentation();
...
fn test_message_builder() {
    ...
    assert_eq!(msg.properties().capacity_profile, Some(MessageCapacityProfile::ConstantRate));
...
fn test_message_properties_clone() {
    ...
    assert_eq!(cloned.properties().safely_replayable, original.properties().safely_replayable);
    assert_eq!(cloned.properties().capacity_profile, original.properties().capacity_profile);
}
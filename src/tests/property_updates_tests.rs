...
async fn test_group_wide_property_update() {
    ...
    if let Some(ConnectionProperty::ConnCapacityProfile(profile1)) =
        props1.get("connCapacityProfile")
    {
    ...
    if let Some(ConnectionProperty::ConnCapacityProfile(profile2)) =
        props2.get("connCapacityProfile")
    {
    ...
    if let Some(ConnectionProperty::ConnCapacityProfile(profile3)) =
        props3.get("connCapacityProfile")
    {
...
async fn test_conn_priority_not_shared() {
    ...
    conn1
        .set_property("connPriority", ConnectionProperty::ConnPriority(10))
        .await
        .expect("Should set priority");

    conn2
        .set_property("connPriority", ConnectionProperty::ConnPriority(50))
        .await
        .expect("Should set priority");

    // Verify priorities are NOT shared (connPriority is per-connection)
    if let Some(ConnectionProperty::ConnPriority(priority1)) =
        conn1.get_property("connPriority").await
    {
    ...
    if let Some(ConnectionProperty::ConnPriority(priority2)) =
        conn2.get_property("connPriority").await
    {

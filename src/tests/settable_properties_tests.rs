...
async fn test_recv_checksum_len_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::RecvChecksumLen(coverage)) =
            conn.get_property("recvChecksumLen").await
        {
...
async fn test_conn_priority_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        for priority in priorities {
            ...
            if let Some(ConnectionProperty::ConnPriority(p)) =
                conn.get_property("connPriority").await
            {
...
async fn test_conn_timeout_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::ConnTimeout(timeout)) =
            conn.get_property("connTimeout").await
        {
            match timeout {
...
async fn test_keepalive_timeout_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::KeepAliveTimeout(timeout)) =
            conn.get_property("keepAliveTimeout").await
        {
            match timeout {
...
async fn test_conn_scheduler_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        for scheduler in schedulers {
            conn.set_property(
                "connScheduler",
                ConnectionProperty::ConnScheduler(scheduler),
            )
            .await
            .expect("Should set property");

            if let Some(ConnectionProperty::ConnScheduler(s)) =
                conn.get_property("connScheduler").await
            {
...
async fn test_conn_capacity_profile_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        for profile in profiles {
            conn.set_property(
                "connCapacityProfile",
                ConnectionProperty::ConnCapacityProfile(profile),
            )
            .await
            .expect("Should set property");

            if let Some(ConnectionProperty::ConnCapacityProfile(p)) =
...
async fn test_multipath_policy_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        for policy in policies {
            conn.set_property(
                "multipathPolicy",
                ConnectionProperty::MultipathPolicy(policy),
            )
            .await
            .expect("Should set property");

            if let Some(ConnectionProperty::MultipathPolicy(p)) =
                conn.get_property("multipathPolicy").await
            {
                assert_eq!(p, policy);
            } else {
                panic!("Property not found");
...
async fn test_send_rate_bounds_properties() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        conn.set_property(
            "minSendRate",
            ConnectionProperty::MinSendRate(Some(1_000_000)),
        ) // 1 Mbps
        .await
        .expect("Should set property");
        ...
        conn.set_property(
            "maxSendRate",
            ConnectionProperty::MaxSendRate(Some(10_000_000)),
        ) // 10 Mbps
        .await
        .expect("Should set property");
...
async fn test_recv_rate_bounds_properties() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        conn.set_property(
            "minRecvRate",
            ConnectionProperty::MinRecvRate(Some(2_000_000)),
        ) // 2 Mbps
        .await
        .expect("Should set property");
        ...
        conn.set_property(
            "maxRecvRate",
            ConnectionProperty::MaxRecvRate(Some(20_000_000)),
        ) // 20 Mbps
        .await
        .expect("Should set property");
...
async fn test_group_conn_limit_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        conn.set_property(
            "groupConnLimit",
            ConnectionProperty::GroupConnLimit(Some(10)),
        )
        .await
        .expect("Should set property");

        if let Some(ConnectionProperty::GroupConnLimit(limit)) =
            conn.get_property("groupConnLimit").await
        {
...
async fn test_all_properties_defaults() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::ConnCapacityProfile(profile)) =
            props.get("connCapacityProfile")
        {

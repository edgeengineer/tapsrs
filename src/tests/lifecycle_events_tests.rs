...
async fn test_path_change_on_endpoint_addition() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        conn.add_remote(new_endpoint).await.expect("Should add endpoint");
        ...
        assert!(events.iter().any(|e| matches!(e, ConnectionEvent::PathChange)),
...
async fn test_multiple_lifecycle_events() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        assert!(events.iter().any(|e| matches!(e, ConnectionEvent::Sent { .. })));

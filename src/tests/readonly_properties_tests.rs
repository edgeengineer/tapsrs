...
async fn test_can_send_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        let final_msg = Message::from_string("Final message")
            .with_final(true);
        conn.send(final_msg).await.expect("Should send final message");
...
async fn test_can_receive_property() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::CanReceive(can_receive)) = conn.get_property("canReceive").await {
            assert!(can_receive, "Should be able to receive on established connection");
        } else {
        ...
        if let Some(ConnectionProperty::CanReceive(can_receive)) = conn.get_property("canReceive").await {
            assert!(!can_receive, "Should not be able to receive on closed connection");
        } else {
            panic!("canReceive property not found");
        }
    }).await.expect("Test should complete within timeout");
...
async fn test_unidirectional_send_connection() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::CanSend(can_send)) = conn.get_property("canSend").await {
            assert!(can_send, "Should be able to send on unidirectional send connection");
        } else {
            panic!("canSend property not found");
        }
        
        if let Some(ConnectionProperty::CanReceive(can_receive)) = conn.get_property("canReceive").await {
            assert!(!can_receive, "Should not be able to receive on unidirectional send connection");
        } else {
            panic!("canReceive property not found");
        }
    }).await.expect("Test should complete within timeout");
...
async fn test_unidirectional_receive_connection() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::CanSend(can_send)) = conn.get_property("canSend").await {
            assert!(!can_send, "Should not be able to send on unidirectional receive connection");
        } else {
            panic!("canSend property not found");
        }
        
        if let Some(ConnectionProperty::CanReceive(can_receive)) = conn.get_property("canReceive").await {
            assert!(can_receive, "Should be able to receive on unidirectional receive connection");
        } else {
            panic!("canReceive property not found");
        }
    }).await.expect("Test should complete within timeout");
...
async fn test_send_msg_max_len() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::SendMsgMaxLen(max_len)) = 
            ...
            assert!(max_len.is_none(), "TCP should have no send message size limit");
        } else {
        ...
        if let Some(ConnectionProperty::SendMsgMaxLen(max_len)) = 
            conn.get_property("sendMsgMaxLen").await {
            assert_eq!(max_len, Some(0), "Should return 0 when sending is not possible");
        } else {
            panic!("sendMsgMaxLen property not found");
        }
    }).await.expect("Test should complete within timeout");
...
async fn test_recv_msg_max_len() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        if let Some(ConnectionProperty::RecvMsgMaxLen(max_len)) = 
            ...
            assert!(max_len.is_none(), "TCP should have no receive message size limit");
        } else {
        ...
        if let Some(ConnectionProperty::RecvMsgMaxLen(max_len)) = 
            conn.get_property("recvMsgMaxLen").await {
            assert_eq!(max_len, Some(0), "Should return 0 when receiving is not possible");
        } else {
            panic!("recvMsgMaxLen property not found");
        }
    }).await.expect("Test should complete within timeout");
...
async fn test_all_readonly_properties_exist() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        assert!(props.get("singularTransmissionMsgMaxLen").is_some(), 
            "singularTransmissionMsgMaxLen should exist");
        assert!(props.get("sendMsgMaxLen").is_some(), "sendMsgMaxLen should exist");
        assert!(props.get("recvMsgMaxLen").is_some(), "recvMsgMaxLen should exist");
    }).await.expect("Test should complete within timeout");
}
...
async fn test_readonly_properties_cannot_be_set() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        let readonly_props = vec![
            ("connState", ConnectionProperty::ConnState(ConnectionState::Closed)),
            ...
            ("sendMsgMaxLen", ConnectionProperty::SendMsgMaxLen(Some(2000))),
            ("recvMsgMaxLen", ConnectionProperty::RecvMsgMaxLen(Some(3000))),
        ];
        
        for (key, value) in readonly_props {
            let result = conn.set_property(key, value).await;
            assert!(result.is_err(), "Should not be able to set read-only property: {}", key);
            if let Err(e) = result {
                match e {

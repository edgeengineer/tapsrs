...
use crate::{
    preconnection::new_preconnection, LocalEndpoint, RemoteEndpoint,
...
async fn test_rendezvous_requires_endpoints() {
    ...
    let result = preconn.rendezvous().await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("No remote endpoints"));
}
...
async fn test_rendezvous_simultaneous_connect() {
    ...
    let remote = RemoteEndpoint {
        identifiers: vec![
            EndpointIdentifier::SocketAddress(peer_addr),
        ],
        protocol: None,
    };

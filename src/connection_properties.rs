...
impl ConnectionProperties {
    pub fn new() -> Self {
        ...
        properties.insert("recvChecksumLen".to_string(), 
            ConnectionProperty::RecvChecksumLen(ChecksumCoverage::default()));
        properties.insert("connPriority".to_string(), 
            ConnectionProperty::ConnPriority(100)); // Default: 100
        properties.insert("connTimeout".to_string(), 
            ConnectionProperty::ConnTimeout(TimeoutValue::default()));
        properties.insert("keepAliveTimeout".to_string(), 
            ConnectionProperty::KeepAliveTimeout(TimeoutValue::default()));
        properties.insert("connScheduler".to_string(), 
            ConnectionProperty::ConnScheduler(SchedulerType::default()));
        properties.insert("connCapacityProfile".to_string(), 
            ConnectionProperty::ConnCapacityProfile(CapacityProfile::default()));
        properties.insert("multipathPolicy".to_string(), 
            ConnectionProperty::MultipathPolicy(MultipathPolicy::default()));
        properties.insert("minSendRate".to_string(), 
            ConnectionProperty::MinSendRate(None)); // Unlimited
        properties.insert("maxSendRate".to_string(), 
            ConnectionProperty::MaxSendRate(None)); // Unlimited
        properties.insert("minRecvRate".to_string(), 
            ConnectionProperty::MinRecvRate(None)); // Unlimited
        properties.insert("maxRecvRate".to_string(), 
            ConnectionProperty::MaxRecvRate(None)); // Unlimited
        properties.insert("groupConnLimit".to_string(), 
            ConnectionProperty::GroupConnLimit(None)); // Unlimited
        properties.insert("isolateSession".to_string(), 
        ...
        properties.insert("tcp.userTimeoutChangeable".to_string(), 
            ConnectionProperty::TcpUserTimeoutChangeable(true)); // Default: true
    ...
    pub fn set(&mut self, key: &str, value: ConnectionProperty) -> crate::Result<()> {
        ...
        match key {
            "connState" | "canSend" | "canReceive" | 
            "singularTransmissionMsgMaxLen" | "sendMsgMaxLen" | "recvMsgMaxLen" => {
                return Err(crate::TransportServicesError::InvalidParameters(
    ...
    pub fn update_readonly(&mut self, state: ConnectionState, can_send: bool, can_receive: bool) {
        self.properties.insert("connState".to_string(), 
            ConnectionProperty::ConnState(state));
        self.properties.insert("canSend".to_string(), 
            ConnectionProperty::CanSend(can_send));
        self.properties.insert("canReceive".to_string(), 

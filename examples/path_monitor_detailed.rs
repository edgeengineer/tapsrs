...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for interface in &interfaces {
        by_type.entry(interface.interface_type.clone())
            .or_default()
            .push(interface);
    }
    ...
    for (iface_type, ifaces) in by_type {
        ...
        for interface in ifaces {
            ...
            println!("  Expensive: {}", if interface.is_expensive { "Yes ⚠️" } else { "No ✓" });
